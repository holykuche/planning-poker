...
const lobbyService = container.get<LobbyGrpcService>(GRPC_SERVER_TYPES.LobbyGrpcService);
const memberService = container.get<MemberGrpcService>(GRPC_SERVER_TYPES.MemberGrpcService);

const server = new Server();
server.addService((protoDescriptor.LobbyService as ServiceClientConstructor).service, {
...
server.addService((protoDescriptor.MemberService as ServiceClientConstructor).service, {
...
server.bindAsync(`0.0.0.0:${ process.env.POKER_PORT }`, ServerCredentials.createInsecure(), () => {
    server.start();
    console.log(`The server has successfully started on port ${ process.env.POKER_PORT }`);
});