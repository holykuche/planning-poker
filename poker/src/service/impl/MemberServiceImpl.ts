...
export default class MemberServiceImpl implements MemberService {
  ...
  getById(memberId: number): Promise<Member> {
    return this.memberDAO.getById(memberId).then(member => {
      if (!member) {
  ...
  getMembersLobbyId(memberId: number): Promise<number> {
    return this.memberLobbyXrefDAO.getMembersBinding(memberId).then(lobbyId => {
      if (!lobbyId) {
        return this.getById(memberId).then(member => {
