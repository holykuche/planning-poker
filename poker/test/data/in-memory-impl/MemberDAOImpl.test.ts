...
describe('data/impl/MemberDAOImpl', () => {
  ...
  it('save should send save query to db', () => {
    ...
    return memberDAO.save(member).then(returnedMember => {
  ...
  it('getById should send find query to db', () => {
    ...
    return memberDAO.getById(member.id).then(returnedMember => {
  ...
  it('getByIds should send find query to db many times', () => {
    ...
    return memberDAO.getByIds(members.map(m => m.id)).then(returnedMembers => {
  ...
  it('getByName should send find query to db', () => {
    ...
    return memberDAO.getByName(member.name).then(returnedMember => {
      expect(dbClientMock.find).toBeCalledWith(
        TableName.Member,
        'name',
        member.name
  ...
  it('deleteById should send delete query to db', () => {
    ...
    return memberDAO.deleteById(member.id).then(() => {
  ...
  it('deleteByIds should send delete query to db many times', () => {
    ...
    return memberDAO.deleteByIds(members.map(m => m.id)).then(() => {
      members.forEach(member => {
        expect(dbClientMock.delete).toBeCalledWith(
          TableName.Member,
          'id',
