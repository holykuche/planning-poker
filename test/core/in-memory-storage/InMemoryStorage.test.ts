...
import { InMemoryStorage } from "core/in-memory-storage";
...
interface InMemoryStorageConstructor {
    new(): InMemoryStorage<TestEntity>;
}

describe("core/in-memory-storage" ,() => {

    const testImpl = function(Constructor: InMemoryStorageConstructor) {

        let impl: InMemoryStorage<TestEntity>;
    ...
    describe("with indexBy and primaryKey options", () => {
        ...
        class IndexedStorage extends InMemoryStorage<TestEntity> {
            constructor() {
                super({
        ...
        testImpl(IndexedStorage);
    });

    describe("with primaryKey option", () => {

        class NotIndexedStorage extends InMemoryStorage<TestEntity> {
            constructor() {
                super({
                    primaryKey: "id",
                });
        ...
        testImpl(NotIndexedStorage);
    });

    describe("without any options", () => {

        class WithoutOptionsStorage extends InMemoryStorage<TestEntity> {}

        let impl: WithoutOptionsStorage;

        beforeEach(() => {
            impl = new WithoutOptionsStorage();
        });
