...
export default class MemberLeaveSubscription extends AbstractCallbackQuerySubscription {
    ...
    protected async handle(callbackQuery: CallbackQuery): Promise<void> {
        const member = await this.telegramUserService.getMemberByTelegramUserId(callbackQuery.from.id);
        ...
        await this.telegramMessageService.deleteAllMessagesFromChat(lobbyId, callbackQuery.message.chat.id);
