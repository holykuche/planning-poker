...
import { LobbyService, MemberService, SERVICE_TYPES, TelegramMessageService, TelegramUserService } from "service/api";
...
export default class CancelPokerSubscription extends AbstractMessageSubscription {
    ...
    protected async handle(msg: Message): Promise<void> {
        const member = await this.telegramUserService.getMemberByTelegramUserId(msg.from.id);
        const lobbyId = await this.memberService.getMembersLobbyId(member.id);

        await this.lobbyService.cancelPoker(lobbyId);

        const messages = await this.telegramMessageService.getMessages(lobbyId, TelegramMessageType.Poker);
        await this.telegramMessageService.deleteMessages(lobbyId, TelegramMessageType.Poker);
