...
export default class MemberEnterSubscription extends AbstractMessageSubscription {
    ...
    protected async handle(msg: Message): Promise<void> {
        ...
        const { id: lobbyId } = await this.lobbyService.getByName(lobbyName) || await this.lobbyService.createLobby(lobbyName);
        const { id: memberId } = await this.telegramUserService.createMember(fromTelegramUserToMember(msg.from));

        const eventContext = { chatId, telegramUserId, lobbyId, lobbyName, memberId };

        await this.subscriptionService.subscribe(lobbyId, memberId)
            .then(event => this.resolveLobbyEventHandler(event.type)(eventContext, event.payload))
    ...
    private async membersWasChangedHandler(eventContext: LobbyEventContext, payload: MembersWasChangedLobbyEvent["payload"]): Promise<void> {
        ...
        const message = await this.telegramMessageService.getMessage(lobbyId, chatId, TelegramMessageType.Lobby);
    ...
    private async pokerResultWasChangedHandler(eventContext: LobbyEventContext, payload: PokerResultWasChangedLobbyEvent["payload"]): Promise<void> {
        ...
        const message = await this.telegramMessageService.getMessage(lobbyId, chatId, TelegramMessageType.Poker);
    ...
    private async pokerWasFinishedHandler(eventContext: LobbyEventContext, payload: PokerWasFinishedLobbyEvent["payload"]): Promise<void> {
        ...
        const pokerMessage = await this.telegramMessageService.getMessage(lobbyId, chatId, TelegramMessageType.Poker);
    ...
    private async lobbyWasDestroyedHandler(eventContext: LobbyEventContext): Promise<void> {
        ...
        const lobbyMessage = await this.telegramMessageService.getMessage(lobbyId, chatId, TelegramMessageType.Lobby);
        await this.bot.editMessageReplyMarkup(null, {
            chat_id: chatId,
            message_id: lobbyMessage.messageId,
        });

        const resultMessage = await this.telegramMessageService.getMessage(lobbyId, chatId, TelegramMessageType.Poker);
        if (resultMessage) {
            await this.bot.deleteMessage(chatId, String(resultMessage.messageId));
        }

        await this.telegramMessageService.deleteAllMessagesFromChat(lobbyId, chatId);
