...
describe('data/telegram-data/in-memory-impl/TelegramUserDAOImpl', () => {
  ...
  it('bindTelegramUserWithMember should save telegram user-member xref', () => {
    ...
    telegramUserDAO.bindTelegramUserWithMember(
      xref.telegramUserId,
      xref.memberId
    );
    expect(telegramUserDAO.find('telegramUserId', xref.telegramUserId)).toEqual(
      xref
    );
  ...
  it('getMemberIdByTelegramUserId should return member ID if xref exists', () => {
    ...
    expect(
      telegramUserDAO.getMemberIdByTelegramUserId(xref.telegramUserId)
    ).toBe(xref.memberId);
  });
  ...
  it('getTelegramUserIdByMemberId should return telegram user ID if xref exists', () => {
    ...
    telegramUserDAO.save(xref);
    expect(telegramUserDAO.getTelegramUserIdByMemberId(xref.memberId)).toBe(
      xref.telegramUserId
    );
  });
  ...
  it('unbindTelegramUserFromMember should delete telegram user-member xref', () => {
    ...
    expect(
      telegramUserDAO.find('telegramUserId', xref.telegramUserId)
    ).toBeNull();
